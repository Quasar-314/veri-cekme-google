...
class VeriÇekmeThread(QThread):
    ...
    def arama_ayarla(self, arama_terimleri):
        """Arama terimlerini ayarlar"""
        self.arama_terimleri = arama_terimleri
        print(f"Arama terimleri ayarlandı: {self.arama_terimleri}")

    def koordinatlari_ayikla(self, url: str) -> tuple[float, float]:
        """URL'den koordinatları ayıklar"""
        try:
            koordinatlar = url.split('!3d')[1].split('!4d')
            enlem = float(koordinatlar[0])
            boylam = float(koordinatlar[1].split('!')[0])
            return enlem, boylam
        except Exception as e:
            print(f'Koordinat ayıklama hatası: {str(e)}')
            return 0.0, 0.0
    ...
    def tum_listeyi_yukle(self, tarayici, wait):
        ...
        while True:
            try:
                ...
                if self.veri_sınırı and sonraki_link_sayisi >= self.veri_sınırı:
                    print(f"Veri sınırına ulaşıldı: {self.veri_sınırı}")
                    break
    ...
    def isletme_verilerini_cek(self, tarayici):
        """Google Maps'ten işletme verilerini çeker"""
        try:
            # İşletme adını çek
            try:
                isim = tarayici.find_element(
                    By.CSS_SELECTOR, 'h1.DUwDvf.lfPIob').text.strip()
            except:
                isim = "Bulunamadı"
                print("İsim çekilemedi")

            # Adresi çek
            try:
                adres = tarayici.find_element(
                    By.CSS_SELECTOR, 'button[data-item-id="address"]').text.strip()
            except:
                adres = "Bulunamadı"
                print("Adres çekilemedi")

            # Website'yi çek
            try:
                website = tarayici.find_element(
                    By.CSS_SELECTOR, 'a[data-item-id="authority"]').get_attribute('href')
            except:
                website = "Bulunamadı"
                print("Website çekilemedi")

            # Telefonu çek
            try:
                telefon = tarayici.find_element(
                    By.CSS_SELECTOR, 'button[data-item-id^="phone:tel:"]').text.strip()
            except:
                telefon = "Bulunamadı"
                print("Telefon çekilemedi")

            # Puanı çek
            try:
                puan = float(tarayici.find_element(
                    By.CSS_SELECTOR, 'div.F7nice').text.strip().split()[0].replace(',', '.'))
            except:
                puan = 0.0
                print("Puan çekilemedi")

            # URL'den koordinatları çek
            try:
                current_url = tarayici.current_url
                enlem, boylam = self.koordinatlari_ayikla(current_url)
            except:
                enlem, boylam = 0.0, 0.0
                print("Koordinatlar çekilemedi")

            # İşletme nesnesini oluştur ve döndür
            isletme = İşletme(
                isim=isim,
                adres=adres,
                website=website,
            ...
            print(f"İşletme verisi başarıyla çekildi: {isim}")
            return isletme

        except Exception as e:
            print(f'İşletme verisi çekilirken hata: {str(e)}')
            return İşletme()
    ...
    def run(self):
        if not self.internet_baglantisi_kontrol():
            print("İnternet bağlantısı yok!")
            QMessageBox.warning(None, "Hata", "İnternet bağlantısı bulunamadı! Lütfen bağlantınızı kontrol edin.")
            return
        ...
        try:
                                ...
                                for link in isletme_linkleri:
                                    try:
                                        ...
                                        if href and href not in islenen_linkler:
                                            ...
                                            if isletme.isim != "Bulunamadı":
                                                isletme_listesi.isletme_listesi.append(isletme)
                                                self.tum_isletmeler.append(isletme)
                                                print(f"İşletme başarıyla eklendi: {isletme.isim}")
                                            
                                                # Her işletme eklendiğinde Excel'e kaydet
                                        ...
                                        print(f'İşletme işlenirken hata: {str(e)}')
                                        continue

                                if not yeni_link_bulundu:
                                    print("\nYeni işletme bulunamadı.")
                                    break

                            except Exception as e:
                                print(f'İşletme listesi işlenirken hata: {str(e)}')
                                break

                    except Exception as e:
                        print(f"Arama işlemi başarısız: {str(e)}")
                        continue

                print(f"Toplanan toplam işletme sayısı: {len(self.tum_isletmeler)}")
                self.sinyal_tamamlandi.emit(self.tum_isletmeler)
            ...
            print(f"Chrome sürücüsü başlatılırken hata: {str(e)}")
            QMessageBox.warning(None, "Hata", "Chrome sürücüsü başlatılamadı! Lütfen Chrome'un yüklü olduğundan emin olun.")
