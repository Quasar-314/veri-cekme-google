...
class VeriÇekmeThread(QThread):
    ...
    def run(self):
        ...
        try:
                ...
                try:
                    ...
                    while True:
                        try:
                            ...
                            for link in isletme_linkleri:
                                try:
                                    ...
                                    if href and href not in islenen_linkler:
                                        ...
                                        # Her işletme eklendiğinde UI'ı güncelle
                            ...
                            # Scroll kontrolü ve diğer mantık aynı kalır
            ...
            tarayici.quit()
            self.sinyal_tamamlandi.emit(isletme_listesi.isletme_listesi)
...
class ScraperTab(QWidget):
    ...
    def durumu_guncelle(self, bilgi):
        arama_terimi, sonuc_sayisi, isletme_verisi = bilgi
        self.durum_etiketi.setText(f"İşleniyor: {arama_terimi} - {sonuc_sayisi} sonuç bulundu")
        
        # Yeni satır ekle
        current_row = self.tablo.rowCount()
        self.tablo.insertRow(current_row)
        
        # Verileri tabloya ekle
        for col, veri in enumerate(isletme_verisi):
            self.tablo.setItem(current_row, col, QTableWidgetItem(veri))
